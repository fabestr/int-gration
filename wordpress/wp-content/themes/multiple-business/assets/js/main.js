...
function primaryHeader(){
	...
	onScroll = function(){
		...
		if( MULTIPLEBUSINESS.is_admin_bar_showing && width >= 782 ){
			scroll = scroll+32;
		}
	 	...
	 	if( height ){
	 		if( height >= scroll ){
	 			...
	 			removeClass();
	 			removeCSS();
	 		}else if( MULTIPLEBUSINESS.is_admin_bar_showing && width >= 782 ){
	 			pos = 32;
	 			addClass();
	 			addCSS();
	 		}else{
...
equaleHeight( '.block-service .icon-content-area' );
...
function homeSlider(){
	...
	jQuery(".home-slider").owlCarousel({
    	...
    	rtl: ( MULTIPLEBUSINESS.is_rtl == '1' ) ? true : false,
	});
};
...
jQuery( document ).ready( function(){
	...
	*/
	new scrollToTop({
		markup   : '<a href="#page" class="scroll-to '+ ( MULTIPLEBUSINESS.enable_scroll_top_in_mobile == 0 ? "hidden-xs" : "" )+'" id="go-top"><span class="kfi kfi-arrow-up"></span></a>',
		selector : '#go-top'
	}).init();
    ...
    */
    $( '#masthead #s' ).attr( 'placeholder', MULTIPLEBUSINESS.search_placeholder );
    $( '#searchform #s' ).attr( 'placeholder', MULTIPLEBUSINESS.search_default_placeholder );
});
